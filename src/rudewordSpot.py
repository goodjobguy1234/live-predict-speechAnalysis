...
class RudeWordSpot:
    def __init__(self, model, waveform_rate, commands = ['กู', 'ควาย', 'คัวย', 'ดอก', 'มึง', 'สัต', 'เสือก', 'หี', 'เหี้ย']):
        self.model = model
        self.waveform_slide = []
        self.result = []
        self.collasp_val = []
    ...
    def preProcess(self, sample_rate, slideSecondList, audio_data):
        windows_size = calculatePadding(sample_rate= sample_rate, sec= slideSecondList)
        waveform = audio_data
        self.waveform_slide, lastest_endTime = slide_window_for_live(waveform, windows_size, self.begin_time)
        self.begin_time = lastest_endTime
